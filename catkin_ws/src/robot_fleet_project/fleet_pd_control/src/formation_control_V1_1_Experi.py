...
#2019-07-18
#改進了pd控制對cmdlist的影響策略，現在一次pd控制修正後的cmd_list會持續有效，直到下一次pd控制介入，或者cmd_list中的數據用完。
...
#--------------------
msg = """
Hello World, hola kevin, fak BaiWan . 
"""
...
class CmdMsgList():
    def __init__(self,initList = None):
        ...
        if initList == None:
            ...
            self.leaderCmdList = np.append(self.leaderCmdList,[[0.3,0]]*35,axis=0) # 0.9 meter
...
def pose_estimate_callback(pose):
    ...
    durationTime= endTime-stratTime
    executeTime.append(durationTime)
...
def shutdownHook():
    global executeTime
    print('shutting down...')
    fo = open("executionTime.txt", "w")
    for item in  executeTime:
        fo.write(str(item)+'\n')
    fo.close()
